...
import { obtenerDatosReporteResponsables } from "~~/server/utils/repositories/reportes";

// Define el manejador de la solicitud para obtener los datos del reporte de responsables
export default defineEventHandler(async (event) => {
  ...
  try {
    // Llama a la función obtenerDatosReporteResponsables para obtener los datos del reporte
    const datosGrafico = await obtenerDatosReporteResponsables();

    // Devuelve los datos obtenidos para ser usados en el gráfico o reporte
    return {
      data: datosGrafico, // Devuelve los datos en un objeto con la propiedad "data"
    };
  } catch (error) {
