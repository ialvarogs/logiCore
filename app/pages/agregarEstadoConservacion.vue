...
// Declara una variable reactiva para almacenar el estado de conservación
const estadoConservacion = ref("");
...
// Función asíncrona para agregar un nuevo estado de conservación
async function agregarEstadoConservacion() {
  try {
    // Limpia cualquier mensaje de error previo
    errorMensaje.value = "";

    // Envía una solicitud POST a la API para agregar el estado de conservación
    const response = await $fetch("/api/estadoConservacion", { // URL del endpoint de la API
      method: "POST", // Método HTTP para crear datos
      body: {
        estado: estadoConservacion.value, // Envia el estado de conservación como datos del cuerpo
      },
    });

    // Si la solicitud es exitosa, imprime la respuesta en la consola
    console.log("Estado de conservación agregado:", response);
    // Redirige a la página de listado de estados de conservación
    router.push("/conservaciones");
  } catch (error) {
    // Si ocurre un error, almacena el mensaje en la variable reactiva
    errorMensaje.value = error.message;
    // Imprime el error en la consola para depuración
    console.error("Error al agregar el estado de conservación:", error.message);
  }
}
</script>
  ...
  <div class="font-sans text-gray-900 antialiased">
    ...
    <div class="flex flex-col sm:justify-center items-center pt-50 sm:pt-50">
      ...
      <div class="w-full sm:max-w-md mt-3 px-6 py-6 bg-white shadow-md overflow-hidden sm:rounded-lg">
        ...
        <form @submit.prevent="agregarEstadoConservacion">
          <!-- Título del formulario -->
          <div class="py-2">
            <span class="text-2xl font-semibold">Agregar Estado de Conservación</span>
          </div>

          <!-- Campo para ingresar el nombre del estado de conservación -->
          <div class="py-2">
            <label class="block font-medium text-sm text-gray-700" for="estadoConservacion">
              Nombre del Estado de Conservación:
            </label>
            <!-- Entrada de texto vinculada a la variable reactiva `estadoConservacion` -->
            <input
              v-model="estadoConservacion"
              type="text"
